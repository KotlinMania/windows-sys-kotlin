# Claude Code Project Instructions — windows-sys-kotlin

## Project Overview

This is **windows-sys-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`windows-sys`](https://crates.io/crates/windows-sys).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/windows-sys/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/windows-sys>
